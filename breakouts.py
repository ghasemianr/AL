...
def detect_breakout_phases(df, avg_body_lookback=100, factor=3.5, 
                           body_factor_pair=2, shadow_pct=0.25):
    """تشخیص فازهای بریک‌اوت:
    - حالت قبلی: حداقل یک کندل با بدنه > factor * میانگین
    - حالت جدید: دو کندل متوالی هر دو با بدنه > body_factor_pair * میانگین، هم‌جهت،
      و شدو سمت جهت (برای صعودی شدو بالا، برای نزولی شدو پایین) کمتر از shadow_pct ارتفاع کندل
    سپس کل رشته هم‌جهت به عنوان فاز ثبت می‌شود.
    """
    if len(df) < 2:
        return []
    ...
    while i < n:
        ...
        # --- شرط قبلی: وجود کندل خیلی بزرگ (factor) ---
        has_large = False
        ...
        # --- شرط جدید: وجود دو کندل متوالی با بدنه > body_factor_pair * میانگین و شرط شدو ---
        has_pair = False
        if not has_large:
            for j in range(start, i-1):
                # بررسی بدنه هر دو کندل
                if (body.iloc[j] > body_factor_pair * rolling_avg.iloc[j]) and \
                   (body.iloc[j+1] > body_factor_pair * rolling_avg.iloc[j+1]):
                    
                    # بررسی شدو بر اساس جهت
                    if dir_val == 1:  # صعودی
                        # کندل j
                        candle = df.iloc[j]
                        ...
                        if upper_shadow / height >= shadow_pct:
                            continue
                        # کندل j+1
                        candle2 = df.iloc[j+1]
                        height2 = candle2['high'] - candle2['low']
                        if height2 == 0:
                            continue
                        upper_shadow2 = candle2['high'] - max(candle2['close'], candle2['open'])
                        if upper_shadow2 / height2 < shadow_pct:
                            has_pair = True
                            break
                    else:  # نزولی
                        # کندل j
                        ...
                        if lower_shadow / height >= shadow_pct:
                            continue
                        # کندل j+1
                        candle2 = df.iloc[j+1]
                        height2 = candle2['high'] - candle2['low']
                        if height2 == 0:
                            continue
                        lower_shadow2 = min(candle2['close'], candle2['open']) - candle2['low']
                        if lower_shadow2 / height2 < shadow_pct:
                            has_pair = True
                            break
        
        if has_large or has_pair:
